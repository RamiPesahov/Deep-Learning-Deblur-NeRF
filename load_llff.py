...
def _load_data(args, basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    num_views_cfg = getattr(args, "num_input_views", -1)

    num_pose_views = poses.shape[-1]
    num_img_views = len(imgfiles)

    if num_views_cfg > 0:
        use_views = min(num_views_cfg, num_pose_views, num_img_views)
    else:
        use_views = min(num_pose_views, num_img_views)

    if num_pose_views != num_img_views:
        print(f"[Info] Mismatch between imgs {num_img_views} and poses {num_pose_views}. Using first {use_views} matching views.")

    imgfiles = imgfiles[:use_views]
    poses = poses[:, :, :use_views]
    bds = bds[:, :use_views]
...
def spherify_poses(poses, bds):
    ...
    def min_line_dist(rays_o, rays_d):
        ...
        b_i = -A_i @ rays_o
        return np.squeeze(-np.linalg.inv((np.transpose(A_i, [0, 2, 1]) @ A_i).mean(0)) @ (b_i).mean(0))
...
def load_llff_data(args, basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_epi=False):
    poses, bds, imgs = _load_data(args, basedir, factor=factor)
    ...
    if spherify:
        ...
        focal = mean_dz * args.render_focuspoint_scale
