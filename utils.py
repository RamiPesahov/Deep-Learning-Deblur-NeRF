...
def get_bbox3d_for_llff(poses, hwf, near=0.0, far=1.0):
    ...
    # force everything onto CPU for bbox computation
    cpu_device = torch.device("cpu")

    # ray directions in camera coordinates
    directions = get_ray_directions(H, W, focal).to(cpu_device)

    min_bound = [100, 100, 100]
    max_bound = [-100, -100, -100]

    poses = torch.tensor(poses, dtype=torch.float32, device=cpu_device)
    ...
    for pose in poses:
        ...
        def find_min_max(pt):
            for i in range(3):
                if min_bound[i] > pt[i]:
                    min_bound[i] = pt[i].item()
                if max_bound[i] < pt[i]:
                    max_bound[i] = pt[i].item()
    ...
    return (
        torch.tensor(min_bound, dtype=torch.float32, device=cpu_device) - torch.tensor([0.1, 0.1, 0.0001], dtype=torch.float32, device=cpu_device),
        torch.tensor(max_bound, dtype=torch.float32, device=cpu_device) + torch.tensor([0.1, 0.1, 0.0001], dtype=torch.float32, device=cpu_device),
    )
...
def get_voxel_vertices(xyz, bounding_box, resolution, log2_hashmap_size):
    ...
    box_min = box_min.to(device=xyz.device, dtype=xyz.dtype)
    box_max = box_max.to(device=xyz.device, dtype=xyz.dtype)

    if not torch.all(xyz <= box_max) or not torch.all(xyz >= box_min):
        xyz = torch.clamp(xyz, min=box_min, max=box_max)

    resolution = torch.as_tensor(resolution, device=xyz.device, dtype=xyz.dtype)
